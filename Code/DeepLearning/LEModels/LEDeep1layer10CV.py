...
#df = pd.read_csv(r"C:\Users\jeffu\Documents\LE50Data.csv")
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path='Deep1L10checkpointLE.pt', trace_func=print):
        """
        Args:
...
def train_model(mod,num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if early_stopping.early_stop:
            ...
            break
    model.load_state_dict(torch.load('Deep1L10checkpointLE.pt'))
    end = time()
...
for i,(train,test) in enumerate(cv.split(X,y)):
    ...
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.AdamW(model.parameters(),lr =.0003, weight_decay = 0.0)
...
metricdf.to_csv('1LayerMetricsLEV4.csv')
