...
class FluxData(Dataset):
    def __init__(self, x, y):
        ...
        # Identify indices of positive samples
        negative_indices = torch.where(self.y_data < 0)[0]

        # Oversampling factor (e.g., replicate positive samples 3 times)
        oversample_factor = 2

        # Oversample positive samples
        self.oversampled_indices = torch.cat([negative_indices] * oversample_factor)

    def __getitem__(self, index):
        if index >= self.n_samples:
            # Select oversampled positive sample
            index = self.oversampled_indices[index % self.n_samples]
    ...
    def __len__(self):
        return len(self.oversampled_indices) if self.oversampled_indices is not None else self.n_samples
#Define simple feed forward model
class Linear(nn.Module):
    def __init__(self, n_features):
        super(Linear,self).__init__()
        #Define the Layers
        self.input = nn.Linear(n_features,256)
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path='Deep1layerLOGOcheckpointLE.pt', trace_func=print):
        """
        Args:
...
def train_model(mod,num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            train_loss.append(mean_squared_error(y_pred1.cpu(),y_train.cpu()))
            test_loss.append(mean_squared_error(y_pred2.cpu(),y_test.cpu()))
            train_r2.append(r2_score(y_train.cpu(),y_pred1.cpu()))
            test_r2.append(r2_score(y_test.cpu(),y_pred2.cpu()))
        ...
        if early_stopping.early_stop:
            ...
            break
    model.load_state_dict(torch.load('Deep1layerLOGOcheckpointLE.pt'))
    end = time()
...
for i, sitename in zip(iter,uni):
    ...
    x_train = df_train.drop(['LE50','Site'],axis=1)
    y_train = df_train['LE50']

    x_test = df_test.drop(['LE50','Site'],axis=1)
    y_test = df_test['LE50']
...
metricdf.to_csv('1LayerLOGOMetricsLEV4.csv')
